    ...
    if st.session_state.sim_stage == "CLOSING":
        ...
        if current_session_history:
            ...
            with download_col4:
                try:
                    import json
                    json_data = json.dumps(current_session_history, ensure_ascii=False, indent=2)
                    st.download_button(
                        label=L.get("download_history_json", "📥 이력 다운로드 (JSON)"),
                    ...
                    st.error(L.get("json_download_error", "JSON 다운로드 오류: {error}").format(error=e))
            ...
            with download_col5:
                try:
                    ...
                    st.download_button(
                        ...
                        data=csv_data.encode('utf-8-sig'),  # BOM 추가로 Excel 호환성 향상
                        file_name=f"chat_history_{st.session_state.sim_instance_id}.csv",
                        mime="text/csv",
                        key="download_chat_csv_file"
                    )
                except Exception as e:
                    st.error(L.get("csv_download_error", "CSV 다운로드 오류: {error}").format(error=e))
        else:
...
# ========================================
    ...
    if st.session_state.call_sim_stage in ["WAITING_CALL", "RINGING"]:
        ...
        with col_in:
            if st.button(L["button_answer"], key=f"answer_call_btn_{st.session_state.sim_instance_id}"):
                ...
                st.rerun()  # ⭐ 전화 응답 버튼 클릭 시 바로 IN_CALL 상태로 전환
        ...
        if st.session_state.call_active:
            ...
            with audio_col2:
                ...
                # ⭐ 수정: 첫 인사말이 아닌 경우에만 여기서 처리 (첫 인사말은 위에서 이미 처리됨)
                
                # 전사 후 고객 반응 생성 처리 (첫 인사말이 아닌 경우)
        ...
        def transfer_session(target_lang: str, current_messages: List[Dict[str, str]]):
            # 언어 이관 시스템 메시지를 추가하고 세션 언어를 변경합니다.
            # ⭐ 전화 통화는 끊어지고 새 언어 팀원으로 통화 화면이 넘어갑니다.
            ...
            if not st.session_state.is_llm_ready:
                ...
                # ⭐ [수정] 전화 통화 종료 처리 - 이관 시 통화가 끊어지고 새 언어 팀원으로 넘어감
                if st.session_state.call_sim_stage == "IN_CALL":
                    # 통화 종료 처리
                    if "call_handler" in st.session_state and st.session_state.call_handler:
                        try:
                            call_id = st.session_state.get("current_call_id", "unknown")
                            user_id = st.session_state.get("user_id", "user")
                            st.session_state.call_handler.end_call(user_id, call_id)
                        except:
                            pass
                ...
                with st.spinner(L["transfer_loading"]):
                    ...
                    try:
                        ...
                        # ⭐ [수정] 전화 탭의 경우 WAITING_CALL로 초기화하여 새 통화 시작
                        if st.session_state.call_sim_stage == "WAITING_CALL":
                            st.session_state.sim_stage = "WAIT_FIRST_QUERY"
                        else:
                            # 채팅 탭의 경우 AGENT_TURN으로 이동
                            st.session_state.sim_stage = "AGENT_TURN"
                        # ⭐ 재실행 불필요: 상태 변경은 이미 반영됨, 다음 렌더링에서 자동 표시됨
                        # st.rerun()
                    except Exception as e:
    ...
    # ========================================
        ...
        # ⭐ 추가: 현재 세션 이력 다운로드 기능 (채팅/이메일과 동일) - JSON/CSV 추가
        st.markdown("**📥 현재 세션 이력 다운로드**")
        download_col1, download_col2, download_col3, download_col4, download_col5 = st.columns(5)
            ...
            with download_col4:
                try:
                    ...
                    st.download_button(
                        label=L.get("download_history_json", "📥 이력 다운로드 (JSON)"),
                        data=json_data.encode('utf-8'),
                        file_name=f"call_history_{st.session_state.sim_instance_id}.json",
                        mime="application/json",
            ...
            with download_col5:
                try:
                    ...
                    st.download_button(
                        label=L.get("download_history_csv", "📥 이력 다운로드 (CSV)"),
                        data=csv_data.encode('utf-8-sig'),  # BOM 추가로 Excel 호환성 향상
                        file_name=f"call_history_{st.session_state.sim_instance_id}.csv",
                        mime="text/csv",

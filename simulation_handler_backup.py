...
def generate_agent_response_draft(current_lang_key: str) -> str:
    ...
    if all_customer_messages:
        ...
        # ⭐ 추가: 고객의 짧은 답변 감지 및 정보 요청 필요성 판단
...
3. **[CRITICAL: Short Response Handling]** If your previous response was very short (e.g., "네", "예", "아니요", "Yes", "No") and the agent asks for more information or clarification:
    - You MUST provide the requested information actively and in detail
    - Do NOT just repeat "네" or "예" - provide the actual information the agent is asking for
    - If the agent asks "어떤 부분이 궁금하신가요?" (What part are you curious about?), you MUST explain your specific question or concern
    - If the agent asks for details, you MUST provide those details, not just acknowledge the request
...
def generate_customer_reaction_for_call(current_lang_key: str, last_agent_response: str) -> str:
    ...
    # ⭐ 수정: 에이전트가 종료 확인 질문을 했는지 확인 (더 엄격하게)
    closing_msg = L_local['customer_closing_confirm']
    # 명시적으로 "다른 문의", "추가 문의"를 물어볼 때만 종료 확인 질문으로 판단
    is_closing_question = (
        closing_msg in last_agent_response or 
        any(phrase in last_agent_response for phrase in [
            "다른 문의가 있으신가요", "추가 문의가 있으신가요", "다른 도움이 필요하신가요",
            "anything else", "other questions", "any other inquiries",
            "다른 문의 사항", "추가 문의 사항"  # 명시적으로 "문의 사항"을 물어볼 때만
        ])
    )
    
    # ⭐ 추가: 에이전트가 초기 문의에 대한 상세 설명을 요청했는지 확인
    ...
    # ⭐ 추가: 초기 문의 내용 포함 (에이전트가 문의 상세 설명을 요청할 때 참조용)
...
- DO NOT change the topic - answer the agent's question directly
...
2. **If agent asked a question** → Answer it SPECIFICALLY and DIRECTLY. Do not give vague or unrelated answers.
   - **CRITICAL**: If the agent asks about your inquiry details (e.g., "문의 내용을 상세히 설명해주세요", "어떤 문의인가요?"), you MUST explain your INITIAL INQUIRY in detail
   - DO NOT mention "추가 문의 사항" or "other inquiries" unless the agent explicitly asks "다른 문의가 있나요?"
   - Answer the agent's question directly - if they ask about your inquiry, explain your inquiry, not something else
3. **If agent requested information** → Provide the EXACT information requested. Be precise and relevant.
   - If your previous response was very short (e.g., "네", "예", "아니요", "Yes", "No") and the agent asks for more details, you MUST provide those details actively
   - Do NOT just say "네" or "예" again - provide the actual information the agent is asking for
   - If the agent asks "어떤 부분이 궁금하신가요?" (What part are you curious about?), you MUST explain your specific question or concern in detail
   - **CRITICAL**: If the agent asks "문의 내용을 상세히 설명해주세요", you MUST provide detailed explanation of your INITIAL INQUIRY: "{initial_inquiry if initial_inquiry else 'your original inquiry'}"
4. **If agent gave a solution or instruction** → Acknowledge it clearly and indicate if you understand or need clarification, based on your emotional state ({customer_emotion})
   - If the solution is clear: "네, 이해했습니다. 감사합니다." or "알겠습니다. 그렇게 진행하겠습니다."
   - If you need clarification: "잠깐, [구체적인 질문]이 궁금합니다." or "혹시 [구체적인 부분]에 대해 더 설명해주실 수 있나요?"
   - DO NOT say "없습니다. 감사합니다" when the agent is explaining or providing solutions - you are in the middle of a conversation
...
7. **CRITICAL - ANSWER THE AGENT'S QUESTION DIRECTLY**:
   - If the agent asks "문의 내용을 상세히 설명해주세요" → Explain your INITIAL INQUIRY in detail
   - If the agent asks "어떤 문의인가요?" → Describe your INITIAL INQUIRY
   - DO NOT say "추가 문의 사항도 있습니다" unless the agent explicitly asks "다른 문의가 있나요?"
   - DO NOT change the topic - answer what the agent asked
   - When the agent asks about your inquiry, explain your INITIAL INQUIRY, not something else
...
10. **CRITICAL - NO CLOSING PHRASES**: 
    - DO NOT say "없습니다. 감사합니다" (No, thank you) unless the agent explicitly asks if you have any other questions
    - DO NOT end the conversation prematurely - continue naturally based on what the agent said
    - If the agent is explaining, asking questions, or providing solutions, respond accordingly and continue the conversation
11. **CRITICAL - NO TOPIC SWITCHING**:
    - If the agent asks about your inquiry, explain your inquiry - do NOT mention other topics
    - "추가 문의 사항도 있습니다" should ONLY be used when the agent asks "다른 문의가 있나요?"
12. **Your response must be a direct answer to the agent's last message above. Read it carefully and respond accordingly.**
...
"""
    try:
        ...
        # ⭐ 추가: 종료 메시지 및 동문서답 필터링 (종료 확인 질문이 아닐 때)
        if not is_closing_question:
            no_more_keywords = [
                "없습니다", "감사합니다", "No, that will be all", "no more",
                "추가 문의 사항 없습니다", "no additional", "結構です"
            ]
            
            # ⭐ 추가: "추가 문의 사항도 있습니다" 같은 동문서답 감지
            ...
            # 에이전트가 설명, 질문, 솔루션 제공 중인지 확인
            agent_is_explaining = any(keyword in last_agent_text.lower() for keyword in [
                "설명", "안내", "도와", "해결", "방법", "절차", "진행", "확인", "문의",
                "explain", "help", "solution", "process", "procedure", "confirm", "inquiry"
            ])
            
            # ⭐ 추가: 에이전트가 문의 상세 설명을 요청했는데 "추가 문의"라고 답변하는 경우 재생성
            if has_additional_inquiry_mention and agent_asking_for_details:
                retry_prompt = f"""
You are a CUSTOMER in a phone call. The agent just asked: "{last_agent_text}"

**CRITICAL ERROR DETECTED**: You mentioned "추가 문의 사항도 있습니다" (additional inquiries) but the agent is asking about your INITIAL INQUIRY details, not about additional inquiries.

**YOUR INITIAL INQUIRY**: "{initial_inquiry}"

**WHAT YOU MUST DO:**
1. The agent asked you to explain your inquiry in detail: "{last_agent_text}"
2. You MUST explain your INITIAL INQUIRY: "{initial_inquiry}"
3. Mention only when customer said "ok, thanks" after solutions were provided from agents properly - "추가 문의 사항" or "다른 문의" - answer the agent's question directly
4. Provide detailed explanation of your initial inquiry based on: "{initial_inquiry}"
5. Focus ONLY on explaining your initial inquiry, and answer the query and solutions from agents - do NOT switch topics

**Example good responses:**
- "네, {initial_inquiry}에 대해 문의드리고 싶어서 연락드렸습니다. [상세 설명]"
- "제가 문의하고 싶은 것은 {initial_inquiry}입니다. [구체적인 내용]"
- "네, {initial_inquiry} 관련해서 [구체적인 문제/질문]이 있어서 연락드렸습니다."

**CRITICAL**: Answer the agent's question about your inquiry. Do NOT mention additional inquiries.

Your response (in {lang_name}, explain your INITIAL INQUIRY in detail):
"""
                reaction = run_llm(retry_prompt)
                reaction_text = reaction.strip()
                
                # 재생성 후에도 여전히 "추가 문의"가 포함되어 있으면 강제로 초기 문의 설명으로 변경
                if any(phrase in reaction_text for phrase in additional_inquiry_phrases):
                    # 강제로 초기 문의 설명 생성
                    if initial_inquiry:
                        reaction_text = f"네, {initial_inquiry}에 대해 문의드리고 싶어서 연락드렸습니다. {initial_inquiry} 관련해서 좀 더 자세히 설명해드릴까요?"
                    else:
                        reaction_text = "네, 제가 문의하고 싶은 내용에 대해 좀 더 자세히 설명해드리겠습니다."
            
            # 종료 메시지가 나왔는데 에이전트가 설명/질문/솔루션 제공 중이면 재생성
            elif has_no_more and agent_is_explaining:
                retry_prompt = f"""
You are a CUSTOMER in a phone call. The agent just said: "{last_agent_text}"

The agent is explaining, asking questions, or providing solutions. You are in the MIDDLE of a conversation.

**CRITICAL RULES:**
1. DO NOT say "없습니다. 감사합니다" (No, thank you) - you are NOT ending the conversation
2. Respond naturally to what the agent just said:
   - If agent explained something: Acknowledge and ask for clarification if needed, or confirm understanding
   - If agent asked a question: Answer it directly
   - If agent provided a solution: Acknowledge it and indicate if you understand or need more details
3. Keep it conversational and natural (1-2 sentences)
4. Your emotional state: {customer_emotion} - respond with {emotion_tone} tone

Example good responses:
- "네, 이해했습니다. 그런데 [구체적인 질문]이 궁금합니다."
- "알겠습니다. 그렇게 진행하면 되나요?"
- "감사합니다. 혹시 [구체적인 부분]에 대해 더 설명해주실 수 있나요?"

Your response (in {lang_name}, respond naturally to the agent's message):
"""
                reaction = run_llm(retry_prompt)
                reaction_text = reaction.strip()
                
                # 재생성 후에도 여전히 종료 메시지만 있으면 강제로 변경
                if any(keyword in reaction_text for keyword in no_more_keywords):
                    # 강제로 적절한 응답 생성
                    if "질문" in last_agent_text or "question" in last_agent_text.lower():
                        reaction_text = "네, 이해했습니다. 감사합니다."
                    elif "설명" in last_agent_text or "explain" in last_agent_text.lower():
                        reaction_text = "알겠습니다. 그런데 좀 더 자세히 설명해주실 수 있나요?"
                    else:
                        reaction_text = "네, 알겠습니다. 감사합니다."
...
def generate_customer_reaction_for_first_greeting(current_lang_key: str, agent_greeting: str, initial_query: str) -> str:
    ...
    # ⭐ 수정: customer_avatar가 없으면 기본값 사용
...
- DO NOT say "없습니다. 감사합니다" (No, thank you) or similar closing phrases
- DO NOT end the conversation - you are just starting it
- You are calling to discuss your issue, so mention it naturally
- Keep it brief (1-2 sentences) but make sure to mention your inquiry

**CRITICAL RULES - YOU MUST FOLLOW:**
1. You MUST mention your inquiry/concern: "{initial_query_text}"
2. DO NOT say "없습니다. 감사합니다" (No, thank you) or similar closing phrases
3. DO NOT end the conversation - you are just starting it
4. You are calling to discuss your issue, so mention it naturally
5. Keep it brief (1-2 sentences) but make sure to mention your inquiry
6. Example good responses:
   - "안녕하세요. {initial_query_text}에 대해 문의드리고 싶어서 연락드렸습니다."
   - "네, 안녕하세요. {initial_query_text} 관련해서 도움을 받고 싶습니다."
   - "안녕하세요. {initial_query_text} 때문에 전화드렸습니다."
...
"""
    try:
        ...
        # ⭐ 추가: 종료 메시지가 포함되어 있으면 제거하고 문의 내용 강조
        no_more_keywords = [
            "없습니다", "감사합니다", "No, that will be all", "no more",
            "추가 문의 사항 없습니다", "no additional", "結構です"
        ]
        
        # 종료 키워드가 포함되어 있고, 문의 내용이 언급되지 않은 경우 재생성
        has_no_more = any(keyword in reaction_text for keyword in no_more_keywords)
        ...
        if has_no_more and not has_inquiry_mention:
            # 종료 메시지가 포함되어 있지만 문의 내용이 없으면 재생성
            retry_prompt = f"""
...
5. Your response MUST include your inquiry: "{initial_query_text}"

Example: "안녕하세요. {initial_query_text}에 대해 문의드리고 싶어서 연락드렸습니다."

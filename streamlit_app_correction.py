...
# ========================================
# FIX 1: 로컬 환경 변수 (.env) 로드를 위해 dotenv 사용 (로컬 실행 시 필요)
import os
...
# LangChain Imports
# FIX: ConversationChain을 가장 안정적인 경로인 'langchain.chains'에서 가져오고,
# 나머지 모듈들은 'langchain_community'에서 가져와 경로 충돌을 방지합니다.
...
def json_db_load(path):
    ...
    try:
        with open(path, "r", encoding="utf-8") as f:
            # 타임스탬프 문자열을 datetime 객체로 변환하는 복잡한 로직은 제거하고, 기본 JSON만 로드
            return json.load(f)
    except Exception:
...
# ===============================
# 1. 환경 변수 로드 & OpenAI 초기화
# ===============================
load_dotenv()
OPENAI_KEY = os.getenv("OPENAI_API_KEY") or st.secrets.get("OPENAI_API_KEY") # OpenAI LLM 및 임베딩 사용
...
LANG = {
    "ko": {
        ...
        "warning_no_db_connect": "❌ DB 연결 실패: Firebase Secrets이 없거나 초기화에 실패하여 데이터베이스를 사용할 수 없습니다.",
        "level_options": ["초급", "중급", "고급"],
        "content_options": ["핵심 요약 노트", "객관식 퀴즈 10문항", "실습 예제 아이디어"],
        "button_generate": "콘텐츠 생성",
        "warning_topic": "학습 주제를 입력해 주세요。",
    ...
    "ja": {
        ...
        "score": "スコア",
        "retake_quiz": "クイズを再挑戦",
        "quiz_error_llm": "LLM이 올바른 JSON의 형식 을 읽어 들이지 못함",
...
def _load_service_account_from_secrets():
    ...
    if "FIREBASE_SERVICE_ACCOUNT_JSON" not in st.secrets:
        ...
        if os.environ.get('FIREBASE_SERVICE_ACCOUNT_JSON'):
            ...
            return None, "FIREBASE_SERVICE_ACCOUNT_JSON Secret/Env Var가 누락되었습니다."
    else:
...
# FIX: 환경 변수/Secrets에서 GCS 버킷 이름 로드
def get_gcs_bucket_name():
    # GCS 버킷 이름이 Secrets이나 환경 변수 둘 중 하나라도 없으면 None을 반환
...
def initialize_firestore_admin(L):
    ...
    # FIX: 키가 없으면 초기화 자체를 건너뜁니다.
    if sa_info is None:
        return None, f"❌ {error_message} (로컬 환경으로 전환됨)"

    db_client = None
    try:
        # Check if app is already initialized (important for Streamlit rerun)
        if not firebase_admin._apps:
            # FIX: Firebase Admin SDK 초기화
...
def init_gcs_client(L):
    ...
    # FIX: 버킷 이름이나 서비스 계정 정보가 없으면 초기화 건너뛰기
    ...
    try:
        ...
        # GCS 클라이언트는 GOOGLE_APPLICATION_CREDENTIALS 환경 변수를 통해 인증합니다.
        tmp = tempfile.NamedTemporaryFile(delete=False, suffix='.json')
        # FIX: 서비스 계정 정보 (sa_info)를 임시 파일에 씁니다.
...
def init_openai_client(L):
    ...
    # FIX: 로컬 환경 변수 (.env)와 Streamlit secrets을 모두 사용하도록 변경
    openai_key = st.secrets.get('OPENAI_API_KEY') or os.environ.get('OPENAI_API_KEY')
    if openai_key:
        try:
            # FIX: os.environ.get()에서 찾은 키를 사용하여 클라이언트 초기화
...
def upload_audio_to_gcs(bucket_name: str, blob_name: str, audio_bytes: bytes, content_type: str = 'audio/webm'):
    ...
    if not gcs_client:
        # FIX: GCS 클라이언트가 없으면 오류 대신 예외 발생
        raise RuntimeError(L['gcs_not_conf'])

    # FIX: GCS 클라이언트가 정상적으로 초기화되었을 때만 버킷 사용
...
str, transcript_text: str, meta: dict = None, mime_type: str = 'audio/webm'):
    ...
    # FIX: DB 객체가 None이면 저장 로직을 바로 종료
    if not db:
        st.warning(L.get("warning_no_db_connect", "DB 연결 실패로 음성 기록을 저장할 수 없습니다."))
        return None
    ...
    # GCS 클라이언트와 버킷 이름이 모두 있을 때만 GCS 업로드 시도
    if bucket_name and gcs_client:
        ...
        st.warning(L['gcs_missing'])  # GCS 미설정 경고
...
def delete_audio_record(db, bucket_name, doc_id: str):
    ...
    # FIX: DB 객체가 None이면 삭제 로직을 바로 종료
    if not db:
        st.error(L.get("warning_no_db_connect", "DB 연결 실패로 삭제할 수 없습니다."))
        return False
    ...
    # delete GCS blob
    try:
        # GCS 클라이언트, 버킷 이름, GCS 경로가 모두 있을 때만 삭제 시도
...
def transcribe_bytes_with_whisper(audio_bytes: bytes, mime_type: str = 'audio/webm', lang_code: str = 'en'):
    L = LANG[st.session_state.language]
    openai_client = st.session_state.get('openai_client')  # FIX: 세션 상태에서 클라이언트 로드
    if openai_client is None:
        # API 키가 없어 초기화되지 않았을 경우 Mock 응답
        return f"❌ {L.get('whisper_client_error', 'Whisper API Client가 초기화되지 않았습니다.')}"
...
def save_simulation_history(db, initial_query, customer_type, messages):
    L = LANG[st.session_state.language]
    # FIX: DB 객체가 None이면 저장하지 않고 경고만 표시
    if not db:
        st.sidebar.warning(L.get("warning_no_db_connect", "DB 연결 실패로 이력을 저장할 수 없습니다."));
        return False
...
def delete_all_history(db):
    L = LANG[st.session_state.language]
    # FIX: DB 객체가 None이면 삭제하지 않고 에러 표시
    if not db:
        st.error(L.get("warning_no_db_connect", "DB 연결 실패로 삭제할 수 없습니다."));
        return
...
def load_or_train_lstm():
    ...
    # FIX: look_back 변수를 사용하여 예측하도록 수정
    look_back = 5
...
def get_closing_messages(lang_key):
    if lang_key == 'ko':
        ...
        return {"additional_query": "また、お客様にお手伝いさせて頂けるお問い合わせは御座いませんか？",
...
# FIX: init_openai_client는 클라이언트 객체를 반환해야 하므로, 세션 상태에 저장합니다.
...
# --- LLM 초기화 ---
# FIX: LLM_API_KEY가 os.environ.get()을 통해 로드되었을 수 있습니다.
...
# RAG Index Loading
# FIX: firestore_db가 None인 경우 RAG 인덱스 로드 시도 자체를 건너뜁니다.
if st.session_state.get(
    ...
    # DB 초기화가 안 되었을 경우 RAG도 사용할 수 없음
    st.session_state.firestore_load_success = False
    st.session_state.is_rag_ready = False
# --- LLM 초기화 끝 ---
...
with st.sidebar:
    ...
    # DB & GCS 상태 표시
    if "❌" in st.session_state.db_init_msg:
        st.warning(st.session_state.db_init_msg)  # DB 초기화 실패 메시지 출력
    elif "✅" in st.session_state.db_init_msg:
        st.success(st.session_state.db_init_msg)
    else:
        st.info("DB Secret 미설정 또는 로컬 전환 상태입니다.")  # 키가 없어서 초기화 건너뛴 경우

    if "❌" in st.session_state.gcs_init_msg:
        st.warning(st.session_state.gcs_init_msg)  # GCS 초기화 실패 메시지 출력
    elif "✅" in st.session_state.gcs_init_msg:
        st.success(st.session_state.gcs_init_msg)
    else:
        st.info("GCS Secret 미설정 또는 로컬 전환 상태입니다.")  # GCS 초기화 건너뛴 경우
    ...
    # FIX: DB가 None이어도 RAG는 Mock으로 실행 가능하지만, DB 저장은 건너뜁니다.
        ...
        if st.button(L["button_start_analysis"], key="start_analysis"):
            with st.spinner(L["data_analysis_progress"]):
                ...
                if st.session_state.firestore_db:
                    ...
                    save_success_msg = " (DB 미연결로 인덱스 저장 건너뜀)"
...
if feature_selection == L["voice_rec_header"]:
    ...
    with col_rec_ui:
        ...
        if audio_bytes:
            ...
            if st.button(L['transcribe_btn'], key='transcribe_btn_key_rec'):
                if st.session_state.openai_client is None:  # FIX: 세션 상태 클라이언트 사용
                    st.error(L['openai_missing'])
                else:
            ...
            if st.button(L['save_btn'], key='save_btn_key_rec'):
                if st.session_state.firestore_db is None:
                    # FIX: DB가 None일 때의 에러 메시지를 명확히 함
                    st.error(L.get("warning_no_db_connect", "DB 연결 실패로 저장할 수 없습니다."))
                else:
    ...
    with col_list_ui:
        ...
        if st.session_state.firestore_db is None:
            # FIX: DB가 None일 때의 경고 메시지를 명확히 함
            st.warning(L.get("warning_no_db_connect", "DB 연결 실패로 이력 기능 사용 불가"))
        else:
            try:
                    ...
                    with st.expander(f"[{created_str}] {transcript_snippet}"):
                        ...
                        if colr.button(L['retranscribe'], key=f'retx_{doc_id}'):
                            if st.session_state.openai_client is None:
                                st.error(L['openai_missing'])  # FIX: 세션 상태 클라이언트 사용
                                ...
                                with st.spinner(L['transcribing']):
                                    try:
                                        ...
                                        if st.session_state.firestore_db:
                                            ...
                                            st.warning("DB 미연결 상태로 전사만 업데이트되었습니다.")
                                        st.rerun()
                                    except Exception as e:
                                        st.error(f"{L['error']} {e}")
                            else:
    ...
    with col_delete:
        if st.button(L["delete_history_button"], key="trigger_delete_history_sim"):
            if not db:
                st.warning(L.get("warning_no_db_connect", "DB 연결 실패로 삭제할 수 없습니다."))
            else:
    ...
    if db:
        ...
        # FIX: DB가 None일 때의 경고 메시지를 명확히 함
        st.warning(L.get("warning_no_db_connect", "DB 연결 실패로 이력 로드 기능 사용 불가"))
    ...
    if st.session_state.is_llm_ready or not LLM_API_KEY:  # Use LLM_API_KEY for initial check
        if st.session_state.is_chat_ended:
            ...
            if st.button(L["new_simulation_button"], key="new_simulation"):
                ...
                st.session_state.simulator_memory.clear()
                st.session_state['last_transcript'] = ''
                st.session_state['agent_response_area_text'] = ''  # FIX: 초기화 추가
                st.rerun()
            st.stop()
...
"""
        ...
        if st.session_state.initial_advice_provided and not st.session_state.is_chat_ended:
            ...
            if last_role in ["customer_rebuttal", "customer_end", "supervisor", "customer"]:
                ...
                # FIX: 새 응답 턴이 시작될 때만 agent_response_area_text를 비워줍니다.
                ...
                if col_transcribe.button(transcribe_button_text, key='start_whisper_transcribe'):
                    ...
                    elif st.session_state.openai_client is None:  # FIX: 세션 상태 클라이언트 사용
                        ...
                        with st.spinner(L.get("whisper_processing", "음성 파일을 텍스트로 변환 중...")):
                            try:
                                ...
                                if transcribed_text.startswith("❌"):
                                    ...
                                    # API Key가 Secrets 또는 Env에 설정되어 있는지 확인하여 피드백 메시지 강화
                ...
                agent_response = col_text_area.text_area(
                    ...
                    value=st.session_state.agent_response_area_text,  # FIX: 세션 상태 값 사용
                ...
                if st.button(L.get("send_response_button", "응답 전송"), key="send_agent_response"):
                    if agent_response.strip():
                        ...
                        st.warning(L.get("empty_response_warning", "응답 내용을 입력하세요."))
            ...
            if last_role == "agent_response":
                ...
                if col_end.button(L.get("button_end_chat", "응대 종료 (설문 조사 요청)"), key="end_chat"):
                    closing_messages = get_closing_messages(current_lang_key)

                    st.session_state.simulator_messages.append(
                        {"role": "supervisor", "content": L.get("customer_closing_confirm", "또 다른 문의 사항은 없으신가요?")})
                    st.session_state.simulator_messages.append(
                ...
                if col_next.button(L.get("request_rebuttal_button", "고객의 다음 반응 요청"), key="request_rebuttal"):
                    if not LLM_API_KEY:
                    ...
                    with st.spinner("고객 반응 생성 중..."):
                        ...
                        positive_keywords = ["감사", "thank you", "ありがとう",
                        ...
                        if is_positive_close:
                            ...
                            st.session_state.simulator_memory.chat_memory.add_ai_message(
                                L.get("customer_closing_confirm", "또 다른 문의 사항은 없으신가요?"))
                        else:
                ...
                with st.spinner(f"답변 생성 중..." if st.session_state.language == 'ko' else "Generating response..."):
                    try:
                        ...
                        answer = response.get('answer',
                                              '응답을 생성할 수 없습니다.' if st.session_state.language == 'ko' else 'Could not generate response.')
                        st.markdown(answer)
                        st.session_state.messages.append({"role": "assistant", "content": answer})
                    except Exception as e:
                        st.error(f"챗봇 오류: {e}")
                        st.session_state.messages.append({"role": "assistant",
                                                          "content": "오류 발생" if st.session_state.language == 'ko' else "An error occurred"})
    else:
...
Requested Format: {display_type_text}"""
    ...
    try:
        ...
        with col_score:
            ...
            st.metric("현재 예측 성취도", f"{predicted_score:.1f}{suffix}")
            st.info(f"다음 퀴즈 예상 점수는 약 **{predicted_score:.1f}점**입니다. 학습 성과를 유지하거나 개선하세요!")

        with col_chart:
            fig, ax = plt.subplots(figsize=(8, 4))
            ax.plot(data, label="과거 점수", marker="o")
            ax.plot(len(data), predicted_score, label="예측 다음 점수", marker="*", color="red", markersize=10)

            ax.set_title("LSTM 기반 학습 성취도 변화 추이")
            ax.set_xlabel("시점 (횟수)")
            ax.set_ylabel("점수 (50-95)")
            st.pyplot(fig)

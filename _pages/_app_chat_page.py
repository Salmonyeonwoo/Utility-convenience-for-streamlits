"""
참고용 app.py의 render_chat_page 함수를 그대로 적용 (업데이트 버전)
요청된 모든 필드 및 아이콘 버튼 포함
"""
...
def render_chat_page():
    """채팅 페이지 렌더링 (참고용 app.py 구조 + 요청된 모든 기능)"""
    ...
    with col2:
        if st.session_state.selected_customer_id:
            ...
            if selected_customer:
                ...
                # AI 응답 생성 (고객 답변 올 때 AI 샘플 답변 표시)
                if current_chats:
                ...
                with chat_container:
                    ...
                    if (current_chats and current_chats[-1]['sender'] == 'customer' and
                        ...
                        if st.button("✅ 사용", key=f"use_ai_{customer_id}", use_container_width=True):
                            ...
                            save_chats(chats)  # 즉시 JSON 파일에 저장
                            st.session_state.ai_suggestion = {}
                ...
                with col_send:
                    if st.button("전송", type="primary", use_container_width=True, key=f"send_{customer_id}"):
                        if chat_input:
                            ...
                            chats[customer_id].append(new_message)
                            save_chats(chats)  # 요청사항: 입력된 데이터 바로 JSON 파일에 업데이트

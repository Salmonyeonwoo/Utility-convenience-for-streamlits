# ========================================
# _pages/chat_modules/agent_turn_manual_input.py
# 에이전트 턴 - 수동 입력 처리
# ========================================
...
def handle_manual_agent_input(L, agent_response_input):
    """에이전트 수동 입력 처리"""
    agent_response = None
    ...
    if agent_response:
        if not agent_response.strip():
            ...
            # ⭐ 에이전트 직접 응답 이력 기록 (비동기 처리로 rerun 방지)
            try:
            ...
            # ⭐ st.chat_input 사용 시 버튼 클릭 후 Streamlit이 자동 rerun하므로 불필요한 rerun 제거

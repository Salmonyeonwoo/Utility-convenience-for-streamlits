...
def render_history_management_panel(L, current_lang):
    ...
    if st.button(f"🔄 {L.get('session_reset', '세션 초기화')}", key="reset_all_session_compact", use_container_width=True, help=L.get("session_reset_help", "모든 채팅/통화 응대 기록을 초기화합니다")):
        st.session_state.show_reset_confirm = True
    
    # 이전 이력 로드 (expander로 축소)
    histories = load_simulation_histories_local(current_lang)
    if histories:
        with st.expander(f"📂 {L.get('load_previous_history', '이전 상담 이력 로드 (최근 10건)')}", expanded=False):
            ...
            if filtered_histories:
                ...
                sel_key = st.selectbox(L.get("select_history", "이력 선택"), options=list(options_map.keys()), key="hist_select_compact")
                if st.button(L.get("load", "로드"), key="load_hist_btn_compact", use_container_width=True):
                ...
                st.info(L.get("no_history_to_load", "로드할 이력이 없습니다."))
    
    # 삭제 및 초기화 확인 다이얼로그
    if st.session_state.get("show_delete_confirm", False):
        st.warning(L.get("confirm_delete_all", "⚠️ 모든 이력이 삭제됩니다. 계속하시겠습니까?"))
        if st.button(L.get("confirm_delete_yes", "예, 삭제합니다"), key="confirm_del_yes_compact", use_container_width=True):
    ...
    if st.session_state.get("show_reset_confirm", False):
        st.warning(L.get("confirm_reset_session", "⚠️ 모든 세션이 초기화됩니다. 계속하시겠습니까?"))
        if st.button(L.get("confirm_reset_yes", "예, 초기화합니다"), key="confirm_reset_yes_compact", use_container_width=True):
            st.session_state.simulator_messages = []
            ...
            st.success(L.get("all_session_reset", "✅ 모든 세션이 초기화되었습니다. 채팅이 종료되었습니다."))
        if st.button(L.get("cancel", "취소"), key="confirm_reset_no_compact", use_container_width=True):
            st.session_state.show_reset_confirm = False

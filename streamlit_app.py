...
LANG = {
    ...
    "en": {
        ...
        "agent_response_placeholder": "Respond with text (rebuttal response)", # st.text_input용
        "agent_response_placeholder_chat": "Respond as agent to customer", # st.chat_input fallback용
    ...
    "ja": {
        ...
        "agent_response_placeholder": "テキストで応答 (再反論の対応)", # st.text_input용
        "agent_response_placeholder_chat": "エージェントとして顧客に回答", # st.chat_input fallback용
...
if feature_selection == L["simulator_tab"]: 
    ...
    if st.session_state.is_llm_ready or not API_KEY:
        ...
        if st.session_state.initial_advice_provided and not st.session_state.is_chat_ended:
            ...
            # 에이전트의 응답이 등록된 후(agent_response) 또는 고객의 마지막 반응 후(customer_rebuttal 등)에만 버튼 표시
